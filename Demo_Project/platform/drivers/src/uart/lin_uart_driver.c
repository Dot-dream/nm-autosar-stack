...
static UART_Type * const g_linUartBase[UART_INSTANCE_COUNT] = UART_BASE_PTRS;

/* Table to save UART IRQ enumeration numbers defined in the CMSIS header file */
static const IRQn_Type g_linUartRxTxIrqId[UART_INSTANCE_COUNT] = UART_RX_TX_IRQS;
...
static lin_state_t * g_linStatePtr[UART_INSTANCE_COUNT] = FEATURE_UART_STATE_PTR;

/* Table to save LIN user config structure pointers */
static lin_user_config_t * g_linUserconfigPtr[UART_INSTANCE_COUNT] = FEATURE_UART_STATE_PTR;
...
static uint8_t s_txBuff[UART_INSTANCE_COUNT][8] = {0U};
                                      ...
                                      uint8_t txSize)
{
    ...
    else
    {
        ...
        else
        {
            ...
            {
                s_txBuff[instance][i] = txBuff[i];
            }
            linCurrentState->txBuff = s_txBuff[instance];
            /* Add a place for checksum byte */
            linCurrentState->txSize = (uint8_t)(txSize + 1U);
            linCurrentState->cntByte = 0U;
            linCurrentState->currentNodeState = LIN_NODE_STATE_SEND_DATA;
...
void LIN_UART_DRV_TimeoutService(uint32_t instance)
{
    ...
    switch (state)
    {
        ...
        case LIN_NODE_STATE_SEND_DATA:
            ...
            {
                linCurrentState->timeoutCounter = linCurrentState->timeoutCounter - 1U;
            }
                                                   ...
                                                   uint8_t tmpByte)
{
    ...
    if (linCurrentState->cntByte == linCurrentState->rxSize)
    {
        ...
        linCurrentState->rxBuff -= linCurrentState->rxSize - 1U; /* PRQA S 0488 */
        if (LIN_UART_DRV_MakeChecksumByte(instance,
                                          linCurrentState->rxBuff,
                                                ...
                                                uint8_t tmpByte)
{
    ...
    if (UART_GetStatusFlag(base, UART_TX_DATA_REG_EMPTY) == false)
    {
        ...
        /* callback function to handle Read back error */
        if (linCurrentState->Callback != NULL)
        {
            linCurrentState->Callback(instance, linCurrentState);
        }
